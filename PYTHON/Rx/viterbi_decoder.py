...
def viterbi_decoder(coded_in, G1, G2, R):
    ...
    if R == "1/2":
        if len(puncpat12) > M:
            ...
            if Q*len(puncpat12) < M:
                puncpat_ext = [puncpat_ext_temp, puncpat12[1: (M - (Q * len(puncpat12)))]]

    elif R == "3/4":
        if len(puncpat34) > M:
            puncpat_ext_temp = puncpat34[1:M]
        else:
            Q = m.floor(M/len(puncpat34))
            puncpat_ext_temp = np.tile(puncpat34, (1, Q))
            if Q*len(puncpat34) < M:
                puncpat_ext = [puncpat_ext_temp, puncpat34[1: (M - (Q * len(puncpat12)))]]
    else:
        if len(puncpat23) > M:
            puncpat_ext_temp = puncpat23[1:M]
        else:
            Q = m.floor(M/len(puncpat23))
            puncpat_ext_temp = np.tile(puncpat23, (1, Q))
            if Q*len(puncpat23) < M:
                puncpat_ext = [puncpat_ext_temp, puncpat23[1: (M - (Q * len(puncpat12)))]]
    ...
    for n in range(0, n_states):
        current_state = de2bi(n, k-1)
        trans_val = current_state[0:k-2]
        trans_val_0 = [0] + trans_val
        trans_val_1 = [1] + trans_val
        N0[n] = bi2de(trans_val_0)  # 0 transition
        N1[n] = bi2de(trans_val_1)  # 1 transition
    # Previous States:
    for n in range(0, n_states):
