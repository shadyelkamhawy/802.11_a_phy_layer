import numpy as np
from TX import constants as c
from TX import sig_field
from TX import payload_bits
from TX import scrambler
from TX import conv_enc
from TX import pilot_generator
from TX import interleave_symbs
from TX import modulate_symbs
...
def top_tx(R, MSC, PPDU_length, FC, MAC1, MAC2, MAC3, TIME_STAMP):
    ...
    sig, NCBPS, NDBPS = sig_field.sig_field(R, MSC, PPDU_length)
    ...
    # PAYLOAD
    ##################################
    bits_final, Nsym = payload_bits.payload_bits(FC, MAC1, MAC2, MAC3, TIME_STAMP, NDBPS, PPDU_length)
    # scramble:
    seed = 93
    scrambled_bits = scrambler.scrambler(bits_final, seed)

    # encode:
    bits_encoded = conv_enc.conv_enc(scrambled_bits, R)
    ...
    pilot_polarity = pilot_generator.pilot_generator(int(Nsym + 1))

    for n in range(1, int(Nsym)+1):
        bits_inter = interleave_symbs.interleave_symbs(bits_encoded[((n-1) * NCBPS) + 0: ((n-1) * NCBPS) + NCBPS], NCBPS)
        iq_symb_fft = modulate_symbs.modulate_symbs(bits_inter, pilot_polarity[n], MSC)
